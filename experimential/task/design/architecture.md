...
源码位于 `experimential/task/core/`，可安装 package 位于 `experimential/task/package/`。开发使用 uv packaged application；发布构建用 Nuitka `--mode=standalone` 生成 `package/runtime/linux-x86_64/task-core.dist/`，稳定入口是 `package/bin/task-core`。
