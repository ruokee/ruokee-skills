...
Use `task_log` for durable work activity: findings, decisions, user/external edits, research results, subagent or handoff results, and unfinished session state. Do not log routine reads, every command, temporary plans, or tool chatter. Append corrections instead of rewriting historical WAL.

Use an actor such as `codex:<model>`, `claude:<model>`, `pi:<model>`, or `codex:<subagent>:<model>` when known. Core falls back to `<host>:unknown` semantics when exact model context is unavailable.
