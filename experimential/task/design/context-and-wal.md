...
- subagent 或 handoff 结果；
- 未完成会话的完成内容、阻塞、关键决定和下一步。

不记录每次读取、每条命令、临时计划、tool chatter 或 Core 已自动写出的同一机械事实。
...
会话结束且工作未完成时，至少追加一条简洁 WAL，包含完成内容、当前阻塞或决定和下一步。细节超过一条有用日志时创建普通 handoff/checkpoint 文件，并从 `TASK.md` 或 WAL 指向它。
