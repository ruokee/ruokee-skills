...
需要单独物化语言变体时，从仓库根目录运行：

```bash
uv run --script scripts/install.py install task --variant en --global --target codex
uv run --script scripts/install.py install task --variant zh --global --target codex
```

该入口只安装 Skill；Core 和宿主 adapter 仍来自上面的 plugin/Pi package 安装流程。
